...
void Automata::collision(void){
    for(int i=0; i<Lx; i++){
        std::complex <double> phi_i = juan[i], phi_i_minus_one = juan[(i-1+Lx)%Lx];
        
        juan[(i-1+Lx)%Lx] = j*sin_theta*phi_i_minus_one + uno*cos_theta*phi_i;
        juan[i] = uno*cos_theta*phi_i_minus_one + j*sin_theta*phi_i;
    }
}
